...
import { createLobby, reduce, viewFor } from '@/games/hottake/engine/engine';
import {
  Action,
  ClientMessage,
  GameState,
 ...
 */
const PEER_PREFIX = 'ashiboy-hottake-';
...
export function toAction(playerId: string, msg: ClientMessage): Action | null {
  ...
  switch (msg.t) {
    ...
    case 'answer':
      ...
      return { t: 'answer', id: playerId, text: msg.text };
    case 'vote':
      if (!Number.isInteger(msg.seat)) return null;
      return { t: 'vote', id: playerId, seat: msg.seat };
    case 'advance':
 ...
 * Owns the full GameState (including sealed answers and ballot authorship),
 * applies all actions through the engine, and publishes a personalized
 * PlayerView to each connected device — never the raw state.
 *
 * No host-side timer: both clocks are display-only pressure (expiry never
 * punishes), so there is nothing to enforce.
 */
